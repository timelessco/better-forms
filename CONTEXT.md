...
**Form Short ID** (`forms.shortId`):
A 7-character, base62 (`a-zA-Z0-9`), globally-unique string that identifies a **Form** in its public URL on reform's default domain — `/forms/<shortId>`. Generated by `nanoid` at Form creation, immutable, system-assigned (never user-edited). Distinct from `forms.id` (internal UUID, used as the primary key and in foreign keys) and from **Slug** (`forms.slug`, the per-custom-domain vanity name).
_Avoid_: short code, public id, share token, form code
