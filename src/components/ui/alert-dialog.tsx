...
export const AlertDialogContent = ({
  ...
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Popup
      ...
      data-size={size}
      className={cn(
        "group/alert-dialog-content fixed inset-s-1/2 top-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-background p-4 elevation-xl duration-100 outline-none data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-sm rtl:translate-x-1/2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
        className,
      )}
      {...props}
    />
...
export const AlertDialogFooter = ({ className, ...props }: React.ComponentProps<"div">) => (
  <div
    ...
    className={cn(
      "flex flex-col-reverse gap-2 pt-2 group-data-[size=sm]/alert-dialog-content:grid group-data-[size=sm]/alert-dialog-content:grid-cols-2 sm:flex-row sm:flex-wrap sm:justify-end",
      className,
    )}
    {...props}
  />
